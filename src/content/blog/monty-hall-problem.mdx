---
author: Jon Openshaw
pubDatetime: 2018-03-10T08:00:01Z
title: A "Proof" of the Monty Hall Problem in JS
postSlug: the-monty-hall-problem
featured: true
draft: false
tags:
  - javascript
description: After an unexpected reintroduction to the Monty Hall Problem, I try to test its premise in JavaScript.
---
...
const runMontyHallProblemTests = n => {
  for (var i = 0; i < n; i++) {
    ...
    let initialChoiceDoor = chooseDoor(3),
      ...
      remainingDoors = [0, 1, 2].filter(d => d !== initialChoiceDoor);
  ...
  console.log(
    `${
      (results.wonGames / (results.wonGames + results.lostGames)) * 100
    }% of switches resulted in wins. ${
      (results.lostGames / (results.wonGames + results.lostGames)) * 100
    }% resulted in losses.`
  );
};
```
